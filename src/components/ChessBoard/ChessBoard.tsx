  ...
  const initialBoardState:Readonly <Piece[]> = [
    {
  ...
  export default function ChessBoard() {
    ...
    const [boardHistory, setBoardHistory] = useState<Piece[][]>([[...initialBoardState]]);
    ...
    function dropPiece(e: React.MouseEvent) {
      ...
      const newBoardState = [...pieces];

      setBoardHistory((prevHistory) => {
        const newHistory = prevHistory.slice(0, currentMoveIndex + 1);
        newHistory.push(newBoardState);
        return newHistory;
      });
  
      // Şu anki hamlenin konumunu güncelleyin
      setBoardHistory((prevHistory) => prevHistory.slice(0, currentMoveIndex + 1));

      // Gelecekteki hamleleri temizleyelim
      setBoardHistory((prevHistory) => prevHistory.slice(0, currentMoveIndex + 1));
      ...
      if (!(tileX === gridX && tileY === gridY)) {
        ...
        if (movedPiece) {
          ...
          let moveLog = `${pieceImage} ${convertTheNumbers(gridX + 1)}${
            gridY + 1
          } -> ${convertTheNumbers(tileX + 1)}${tileY + 1}`;
    ...
    function MoveLog({
      ...
      function handleMoveClick(index: number) {
        // Hamle düğmesine tıklandığında, tahtayı ve hamleleri geri alın
        setCurrentMoveIndex(index);
        setPieces(boardHistory[index]);
    ...
    function convertTheNumbers(num) {
      if (num === 1) return "A";
    ...
    return (
      <div
        ...
        ref={ChessBoardRef}
      >
        <button onClick={handleUndoClick} className="button">Geri Al</button>
